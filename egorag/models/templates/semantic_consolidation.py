...
# Consolidation Rules:
1. **Unify Character Names**: Character names might change as more information is revealed (e.g., "person_1" becomes "Anna"). If a new triple uses a more specific name for a character already in the existing triples, update the old triples (by removing and re-adding) or use the most specific name in your updated triple.
2. **Merge Similar Information**: If existing triples express very similar information to the new triple, remove them and update the new triple to capture the most complete/accurate representation
