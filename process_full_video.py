...
import glob
from egorag.models.mllm_gpt import generate_messages, get_response
...
def process_full_video(video_name: str):
    ...
    output_dir = Path("data/memorization")
    ...
    for folder in tqdm(image_folders, desc="Captioning clips"):
        ...
        messages = generate_messages(current_images, CAPTION_PROMPT)
        try:
    ...
    print(f"Memory generation complete for {video_name}! Total tokens used: {total_tokens}")
